        ...
        {filteredCompanies.map((company) => (
          <div
            ...
            className={`bg-white rounded-2xl shadow-sm border-2 p-6 transition-all duration-300 ${
              company.is_active ? 'border-slate-200 hover:shadow-lg' : 'border-slate-200 opacity-60'
            }`}
          >
            ...
            {/* Category Badge */}
            <span
              className={`inline-block px-3 py-1 rounded-full text-xs font-semibold mb-4 ${getCategoryBadge(
                company.category
              )}`}
            >
      ...
      {showEditModal && editingCompany && (
        <CompanyModal
          ...
          onSave={async (data) => {
            ...
            await updateCompany(editingCompany.id, data); // Supabase action
            loadCompanies();
            setShowEditModal(false);
            setEditingCompany(null);
          }}
...
type CompanyModalProps = {
  ...
  onClose: () => void;
  onSave: (data: Omit<InsuranceCompany, 'id'>) => void;
};

function CompanyModal({ company, onClose, onSave }: CompanyModalProps) {
  const [formData, setFormData] = useState<Omit<InsuranceCompany, 'id'>>({
    name: company?.name || '',
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) =>
                setFormData({ ...formData, category: e.target.value as 'general' | 'health' | 'life' })
              }
              className="w-full px-4 py-3 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#004aad] focus:border-transparent"
            >
